...
| DEC-022 | **Partially superseded by DEC-050/DEC-051:** Original baseline Node.js `24.19.0`, pnpm `11.16.0`, Next.js `16.3.0`, React/React DOM `19.2.8`, TypeScript `7.0.2`, Tailwind CSS `4.3.3`, `@tailwindcss/postcss` `4.3.3`, Drizzle ORM `0.45.2`, Drizzle Kit `0.31.10`. | Stable pins as of 2026-08-07; package manager and control-plane runtime later revised. |
...
| DEC-051 | JavaScript package manager and Node-service runtime move from pnpm/Node+tsx to Bun `1.3.14` (workspaces via root `package.json`, `bun.lock`, Docker images `oven/bun:1.3.14-alpine`). Vite UI and remaining TS services install/run with Bun; Go API binary is unchanged. | Lower image-build RAM/time than repeated `pnpm install` on ~1 GB hosts while keeping the existing TypeScript worker/control/node-api code. |
